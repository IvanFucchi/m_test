...
if (!res.ok) {
  console.warn('Eventbrite fallback:', res.status, await res.text());
  return [];          // ⬅︎   restituisce array vuoto, niente crash
}

  const data = await res.json();

  // DEBUG: conta gli eventi grezzi restituiti
  console.log('Eventbrite raw count:', data.events?.length ?? 0, 'items');

  // Trasforma e filtra gli eventi
  ...
  // DEBUG: ispeziona il primo evento mappato
  if (mappedEvents.length > 0) {
    console.log('Mapped events sample:', JSON.stringify(mappedEvents[0], null, 2));
  }
...
if (import.meta && import.meta.url && import.meta.url.endsWith(process.argv[1])) {
  // ESM eseguito direttamente con `node fetch_real_events.js ...`
  (async () => {
    try {
